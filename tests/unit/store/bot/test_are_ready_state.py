...
class TestAreReadyState:
    ...
    async def test_handle_ready_dont_exist_session(
        ...
        """Тест на кнопку "Готов". Но не существует игры"""
        callback.data = "ready"
    ...
    async def test_handle_ready_if_user_not_participant(
        self, are_ready, callback, session_game, full_players, player7
    ):
        """Тест на кнопку "Готов".
        Этот пользователь не игрок сессии
        """
        callback.data = "ready"
    ...
    async def test_handle_ready_if_player_already_true(
        self, are_ready, callback, session_game, full_players, player6
    ):
        """Тест на кнопку "Готов".
        Но игрок уже приготовился
        """
        callback.data = "ready"
    ...
    async def test_handle_ready_player_make_ready(
        self, are_ready, callback, session_game, full_players, player5, player6
    ):
        """Тест на кнопку "Готов".
        Если есть такой игрок, но is_ready=False -
        делает is_ready=True
        """
        callback.data = "ready"
