...
from app.store import FSMContext, GameSessionAccessor, TimerManager, RoundAccessor, PlayerAccessor, Store
...
def callback(
    ...
    callback_ = CallbackTG.from_dict(
        {'chat_instance': '526193150226325798',
         ...
         'message': {'chat': {'id': 123, 'title': 'т', 'type': 'supergroup'},
                     'date': 1760457959,
                     'from': {'first_name': 'Что? Где? Когда?',
                              'id': 7997238478,
                              'is_bot': True,
                              'username': 'quiz_zalim_bot'},
                     'message_id': 41,
                     'reply_markup': {'inline_keyboard': [[{'callback_data': 'start_game',
                                                            'text': '🎮 Начать игру'}],
                                                          [{'callback_data': 'show_rules',
                                                            'text': '📋 Правила'},
                                                           {'callback_data': 'show_rating',
                                                            'text': '⭐ Рейтинг'}]]},
                     'text': 'Добро пожаловать в игру! Используйте кнопки ниже для '
                             'управления.'}}
    )
    return callback_
